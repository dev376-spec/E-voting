...
//retrive candidates from the candidates table
$result=mysql_query("SELECT * FROM voices")
...
$positions_retrieved=mysql_query("SELECT * FROM voices")
or die("There are no records to display ... \n" . mysql_error()); 
if (mysql_num_rows($result)<1){
    $result = null;
}
...
$newCandidateName = addslashes( $_POST['candidate'] ); //prevents types of SQL injection
$newCandidatePosition = addslashes( $_POST['position'] ); //prevents types of SQL injection

$sql = mysql_query( "INSERT INTO voices(candidate,position) VALUES ('$newCandidateName','$newCandidatePosition')" )
...
// check if the 'id' variable is set in URL
 ...
 // delete the entry
 $result = mysql_query("DELETE FROM voices WHERE id='$id'")
...
<CAPTION>
  <h1>MANAGE CANDIDATES</h1></caption>
 
<div id="container">
<table width="380" align="center">
<table width="380" align="center">
<CAPTION><h3>ADD NEW CANDIDATE</h3></CAPTION>
<form name="fmPositions" id="fmPositions" action="candidates.php" method="post" onsubmit="return positionValidate(this)">
<tr>
...
echo "<tr>";
echo "<td>"."&nbsp"."&nbsp"."&nbsp"."&nbsp"."&nbsp" . $row['Candidate']."</td>";
echo "<td>" ."&nbsp"."&nbsp"."&nbsp"."&nbsp"."&nbsp"."&nbsp"."&nbsp"."&nbsp"."&nbsp"."&nbsp"."&nbsp"."&nbsp"."&nbsp".$row['Position']."</td>";
echo '<td><a href="candidates.php?id=' . $row['Id'] . '">Delete Position</a></td>';
echo "</tr>";
}
